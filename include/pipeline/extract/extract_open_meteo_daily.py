...
def fetch_daily() -> pd.DataFrame:
    ...
    for cidade, item in zip(cidades, dados):
        df_cidade = pd.DataFrame(item.get("daily", {}))
...
def save_to_minio(df_total: pd.DataFrame) -> str:
    ...
    for dt_ext, group in df_total.groupby("data_extracao"):
        s3_file_path = f"s3://{bucket}/forecast_daily/data_extracao={dt_ext}/daily_{time_str}.parquet"
...
def update_bronze_view():
    ...
    conn.execute(f"""
        CREATE OR REPLACE VIEW bronze.open_meteo_forecast_daily AS
